...
public class Scale extends AbstractAnimation {
  ...
  public double[] getScaleAtTick(int t) {
    ...
    double width = this.fromShape.getSize()[0] * (this.endTime - t)
            / (this.endTime - this.startTime) +
            this.toShape.getSize()[0] * (t - this.startTime)
                    / (this.endTime - this.startTime);
    double height = this.fromShape.getSize()[1] * (this.endTime - t)
            / (this.endTime - this.startTime) +
            this.toShape.getSize()[1] * (t - this.startTime)
                    / (this.endTime - this.startTime);
