...
public class ChangeColor extends AbstractAnimation {
  ...
  public Color getColorAtTick(int t) {
    if (this.startTime == this.endTime) {
      return new Color(this.toShape.getColor().getR(), this.toShape.getColor().getG(),
          this.toShape.getColor().getB());
    }
    int midR = this.fromShape.getColor().getR() * (this.endTime - t)
            / (this.endTime - this.startTime)
            + this.toShape.getColor().getR() * (t - this.startTime)
            / (this.endTime - this.startTime);
    int midG = this.fromShape.getColor().getG() * (this.endTime - t)
            / (this.endTime - this.startTime)
            + this.toShape.getColor().getG() * (t - this.startTime)
            / (this.endTime - this.startTime);
    int midB = this.fromShape.getColor().getB() * (this.endTime - t)
            / (this.endTime - this.startTime)
            + this.toShape.getColor().getB() * (t - this.startTime)
            / (this.endTime - this.startTime);
